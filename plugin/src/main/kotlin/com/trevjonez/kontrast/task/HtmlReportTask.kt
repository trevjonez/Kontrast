...
open class HtmlReportTask : DefaultTask() {
    ...
    fun invoke() {
        ...
        testCases.timeout(10, TimeUnit.MILLISECONDS)
                .onErrorResumeNext { Single.error(IllegalArgumentException("Html report task is ran automatically after the kontrast test task(s), do not invoke explicitly.")) }
                .map {
                    it.map { testCase ->
